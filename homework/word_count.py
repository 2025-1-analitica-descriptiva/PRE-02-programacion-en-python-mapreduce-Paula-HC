...
def load_input(input_directory):
    ...
    files = glob.glob(f"{input_directory}/*") # busca todos los archivos en el directorio
    with fileinput.input(files=files) as f: # abre los archivos
        for line in f:
            sequence.append((fileinput.filename(), line)) # agrega una tupla a la lista con el nombre del archivo y la linea
    return sequence
...
def line_preprocessing(sequence):  
    ...
    return sequence # devuelve la lista de tuplas con el preprocesamiento realizado
...
def mapper(sequence):
    """Mapper"""
    return [
        (word, 1) for key, value in sequence for word in value.split()  # itera sobre la lista de tuplas y separa las palabras
    ] # devuelve una lista de tuplas con la palabra y el valor 1
...
def shuffle_and_sort(sequence):
    """Shuffle and Sort"""
    sequence.sort(key=lambda x: x[0]) # ordena la lista de tuplas por la clave
    return sequence # devuelve la lista de tuplas ordenada
...
def reducer(sequence):
    """Reducer"""
    result = [] # lista vacia para almacenar las tuplas
    for key, group in groupby(sequence, lambda x: x[0]): # agrupa la lista de tuplas por la clave
        result.append((key, sum(value for _, value in group))) # agrega una tupla a la lista con la clave y la suma de los valores
    return result
